...
var path = require('path'),
    ...
    hashbuild = require('hashsome').hashdirs;
...
var hashdirs = function (builds, cb) {
    var dirs = [];

    Object.keys(builds).forEach(function (module) {
        dirs.push(path.join(builds[module].buildDir, module));
    });

    hashbuild(dirs, function(err, results) {
        log.info('--hashdirs results:\n' + (err || JSON.stringify(results)));
        if (process.send) {
            process.send({cmd: 'hashdirs', results: err || results});
        }
        cb(err, results);
    });
};
    ...
    var stack = new Stack(),
        ...
        buildStat,
            ...
            if (buildCallback) {
                ...
                log.info('lib/builder:start() callback unspecified, calling process.exit(0)');
                process.exit(0);
            }
        },
        post = function (json, callback) {
            ...
            } else if (options.hashdirs && Object.keys(json.builds).length) {
                hashdirs(json.builds, callback);
            } else {
                callback();
            }
        };
