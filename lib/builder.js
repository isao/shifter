...
var hashdirs = function (builds, cb) {
    ...
    Object.keys(builds).forEach(function (module) {
        var dir = path.join(builds[module].buildDir, module);

        if (fs.existsSync(dir)) {
            log.info('enqueueing ' + dir + ' for hash/rename');
            dirs.push(dir);
        } else {
            log.error('build dir "' + dir + '" not found');
        }
    });

    hashsome(dirs, function(err, results) {
        if (err) {
            log.error(err);
            results = null;
            log.error('hashdirs failed: ' + err.message);
        } else {
            log.info('hashdir results\n' + JSON.stringify(results, null, 2));
        }
        cb(err, results);
    });
}
