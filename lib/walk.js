...
exports.run = function (options, callback) {
    ...
    modStack.done(function () {
        ...
        var stack = new Stack(),
            errors = [],
            hashed = {}, // save results from builder.js hashdirs()
            run = function () {
                var mod = mods.pop(), opts, child;
                if (mod) {
                    opts = {
                        cwd: path.join(shifter.cwd(), mod),
                        silent: true
                    };
                    child = fork(util.shifter, args, opts);
                    child.on('message', function (msg) {
                        if (msg && msg.results && (msg.cmd === 'hashdirs')) {
                            Object.keys(msg.results).forEach(function (modname) {
                                hashed[modname] = msg.results[modname];
                            });
                        }
                    });
        ...
        stack.done(function () {
            ...
            if (errors.length) {
                ...
                log.info('lib/walk:run() callback unspecified, calling process.exit(0)');
                process.exit(0);
            }
        });
    });
